...
def fetch_rpg(date_range_label, cfg):
    ...
    # Build all 30 team IDs into one request
    team_ids = ','.join(str(v) for v in MLB_IDS.values())

    if cfg['type'] == 'season':
        url = (
            f"https://statsapi.mlb.com/api/v1/teams/stats"
            f"?season=2026&stats=season&group=hitting&gameType=R"
            f"&teamIds={team_ids}"
        )
    else:
        url = (
            f"https://statsapi.mlb.com/api/v1/teams/stats"
            f"?season=2026&stats=byDateRange&group=hitting&gameType=R"
    ...
    # Build reverse lookup: mlb_id → abbr
    ...
    for split in splits:
        ...
        if not abbr:
            logging.warning(f"  Unknown team ID: {team_id}")
            continue
        ...
        logging.info(f"  {abbr}: {runs} R / {games} G = {rpg} R/G")
