...
START_STR  = start_date.strftime('%Y-%m-%d')   # e.g. 2026-03-06
END_STR    = today.strftime('%Y-%m-%d')         # e.g. 2026-04-05
DATE_RANGE = f"{START_STR} to {END_STR}"
SEASON     = today.year

URL = (
    f"https://www.fangraphs.com/leaders-legacy.aspx?pos=np&stats=bat&lg=all&qual=5"
    f"&type=c,6,34,35,36,23,37,38,39,40,53,41,52,51,50,61"
    f"&season={SEASON}&month=1000&season1={SEASON}&ind=0&team=0&rost=0&age=0"
    f"&filter=&players=0&startdate={START_STR}&enddate={END_STR}"
    f"&v_cr=legacy&page=1_2000"
)
SPLIT_NAME = "No Splits"

logging.info(f"📥 Scraping Last 14 Days — {SPLIT_NAME} ({DATE_RANGE})...")
...
supabase.table("fangraphs_last14_batting").delete().eq("split_type", SPLIT_NAME).execute()
logging.info(f"🗑️  Cleared old rows for '{SPLIT_NAME}'")

supabase.table("fangraphs_last14_batting").upsert(
    records, on_conflict="player,team,split_type,scrape_date"
).execute()

logging.info(f"✅ {len(df):,} rows upserted for Last 14 Days — {SPLIT_NAME} ({DATE_RANGE})")
time.sleep(4)
