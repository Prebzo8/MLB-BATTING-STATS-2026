...
logging.info(f"📥 Scraping Last 7 Days — {SPLIT_NAME} ({DATE_RANGE})...")
...
supabase.table("fangraphs_last7_batting").delete().eq("split_type", SPLIT_NAME).execute()
logging.info(f"🗑️  Cleared old rows for '{SPLIT_NAME}'")

supabase.table("fangraphs_last7_batting").upsert(
    records, on_conflict="player,team,split_type,scrape_date"
).execute()

logging.info(f"✅ {len(df):,} rows upserted for Last 7 Days — {SPLIT_NAME} ({DATE_RANGE})")
time.sleep(4)
