...
RW_ABBR_MAP = {
    # 2-letter or alternate forms → canonical 3-letter
    ...
    # Athletics — Sacramento 2025–27 transition, Las Vegas future
...
DISPLAY_NAME_MAP = {
    ...
    "Cristian Pache":        "Cristian Pachec",
    ...
    def parse_side(ul, side_label):
        ...
        if ph:
            ...
            if not raw_name:
                ...
                # Strip trailing handedness suffix (e.g. " L", " R", " S")
                cleaned = re.sub(r'\s+[LRS]$', '', raw_name).strip()
                pitcher_name = normalize_name(cleaned)

            # Prefer the dedicated .lineup__throws element; fall back to suffix
            ...
            if hand_el:
                ...
                # Fallback: pull hand from the raw name suffix
                m = re.search(r'\s+([LR])$', raw_name)
                if m:
                    pitcher_hand = m.group(1)

            if pitcher_hand is None:
                logging.warning(f"  {side_label} — could not determine pitcher hand (raw='{raw_name}')")
        ...
        logging.info(
            f"  {side_label} | {status} | "
            f"pitcher={pitcher_name} ({pitcher_hand}) | batters={len(order)}"
        )
        return {
            "lineup_status": status,
            "pitcher_name":  pitcher_name,
            "pitcher_hand":  pitcher_hand,
            "batting_order": order,   # pass list directly; Supabase client handles JSON
        }
